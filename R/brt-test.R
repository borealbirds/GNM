...
## 1st subset takes independent points (replace=TRUE)
sset1 <- sample(nrow(ddi), nsub, replace=FALSE, prob=wi)
DATs <- DAT[sset1,]

t0 <- proc.time()
brt2 <- try(gbm.step(DATs,
    gbm.y = 1,
    gbm.x = 4:ncol(DATs),
    offset = DATs$offset, #site.weights = DAT$weights,
    family = "poisson", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
proc.time() - t0 # BCR6/CAWA ~ h
save(sset1, brt2, file="d:/bam/BAM_data_v2019/gnm/tmp/bcr1-sub-pt.Rdata")


pr1 <- raster::predict(r, brt1, type="response", n.trees=brt1$n.trees)
pr2 <- raster::predict(r, brt2, type="response", n.trees=brt2$n.trees)


vi1 <- as.data.frame(brt1$contributions)
cv1 <- t(as.data.frame(brt1$cv.statistics))
vi2 <- as.data.frame(brt2$contributions)
cv2 <- t(as.data.frame(brt2$cv.statistics))

## compare speed with bootstrap resample (n=sum of wi?)
## 1/100 of data leads to 100x speed-up?
