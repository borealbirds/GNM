...
dir <- "d:/bam/BAM_data_v2019/gnm/www/ds/"
...
sp$combo <- paste0(sp$english_name, " (", sp$scientific, ")")
...
fmap <- paste0(dir, spp, "_pred1km1.png")
fdat <- paste0(dir, spp, "_densities.csv")

omap <- paste0(out, spp, "/map.png")
odat <- paste0(out, spp, "/index.json")
...
x$Area <- bcr$area[match(x$BCR_NALC, bcr$BCR_NALC)]
x$N <- 2 * 100 * x$mean * x$Area
x$NQ5 <- 2 * 100 * x$Q5 * x$Area
x$NQ50 <- 2 * 100 * x$Q50 * x$Area
x$NQ95 <- 2 * 100 * x$Q95 * x$Area


if (!dir.exists(paste0(out, spp)))
    dir.create(paste0(out, spp))
file.copy(fmap, omap)
